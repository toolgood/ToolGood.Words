...
public class IllegalWordsSearch extends BaseSearchEx
{
    ...
    private IllegalWordsSearchResult GetIllegalResult(int index, int end, String srcText, int p, int[] pIndex, int flag)
    {
        ...
        if (keyword.length()==1) {
            if(ToSenseWord(srcText.charAt(end)).equals(ToSenseWord(keyword.charAt(0)))==false){ return null; }
            return new IllegalWordsSearchResult(keyword, end, end, srcText);
        }
        int start = FindStart(keyword, end, srcText, p, pIndex);
        if (start == -1) { return null; }
        if(ToSenseWord(srcText.charAt(start)).equals(ToSenseWord(keyword.charAt(0)))==false){ return null; }
        if (UseBlacklistFilter) {
    ...
    protected void Load(FileInputStream  br) throws IOException
    {
        ...
        UseDuplicateWordFilter=br.read()>0?true:false;
        ...
        UseDBCcaseConverter=br.read()>0?true:false;
        UseSimplifiedChineseConverter=br.read()>0?true:false;
