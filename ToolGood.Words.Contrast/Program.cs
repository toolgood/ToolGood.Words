...
namespace ToolGood.Words.Contrast
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            var bits = "";
            for (int i = 0; i < 128; i++) {
                bits += WordTest.getType((char)i).ToString();
            }
            ...
            Run("StringSearch  ToSenseWord", () => { word.ContainsAny(WordHelper.ToSenseWord(text)); });
            ...
            Run("IllegalWordsSearch ", () => { iword1.FindAll(text); });
